...
app.use(cors({
    ...
    allowedHeaders: ['Content-Type', 'Authorization', 'metamask-address', 'metamask-message', 'metamask-signature']
}));
...
app.post('/api/auth', validateMetaMaskCredentials, async (req, res) => {
    ...
    try {
        ...
        const addressType = await checkAddressTypeAsFluenceBackend(req.metamask.address);
